...
function flushPending() {
  ...
  flushPromise = flushPromise.then(async () => {
    ...
    if (store.entries.length > MAX_ENTRIES) store.entries = store.entries.slice(store.entries.length - MAX_ENTRIES);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message?.type === "SUBTOVOICE_LOG_GET") {
    flushPending().then(async () => {
      const result = await chrome.storage.local.get(LOG_KEY);
      sendResponse({ ok: true, log: result[LOG_KEY] || baseStore() });
  ...
  if (message?.type === "SUBTOVOICE_LOG_MARK") {
    ...
    void flushPending().then(() => sendResponse({ ok: true }));
    return true;
  }
});
